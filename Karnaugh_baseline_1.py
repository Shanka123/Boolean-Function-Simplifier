...
class arch(nn.Module):
	def __init__(self):
		...
		self.block1 = nn.Sequential(nn.Conv2d(in_channels=1, out_channels=3, kernel_size=3, stride=1, padding=1), 
			...
			# nn.ConvTranspose2d(in_channels = 1, out_channels = 72, kernel_size = 2, stride = 2), nn.LeakyReLU(0.1), nn.Dropout(0.1))
...
for epoch in range(iterations):
	...
	torch.save(net.state_dict(), 'running_model_1.pt')
	with open('present_epoch_1.txt','w') as f:
		f.write(str(epoch + 1))
	if test_acc > max_test_acc:
		max_test_acc = test_acc
		temp_epoch = epoch + 1
		torch.save(net.state_dict(), 'best_model_1.pt')
...
net = arch()
net.load_state_dict(torch.load('best_model_1.pt', map_location='cpu'))
print('Model loaded !')
net = net.double().cuda()
...
np.save('all_test_acc_1.npy', np.array(testAcc))
np.save('all_val_acc_1.npy', np.array(valAcc))
np.save('all_train_acc_1.npy', np.array(trainAcc))

np.save('all_test_loss_1.npy', np.array(testLoss))
np.save('all_val_loss_1.npy', np.array(valLoss))
np.save('all_train_loss_1.npy', np.array(trainLoss))
# net.train(False)
