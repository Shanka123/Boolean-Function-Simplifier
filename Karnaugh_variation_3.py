...
class arch(nn.Module):
	def __init__(self):
		...
		self.fc = nn.Sequential(nn.Linear(in_features = 256*4*4, out_features = 129, bias = True))
	def forward(self, x):
		# print(x.size())
		# exit()
		x1 = self.pad_1x2(x)
...
for epoch in range(iterations):
	...
	torch.save(net.state_dict(), 'running_new_3_model.pt')
	with open('new_3_present_epoch.txt','w') as f:
		f.write(str(epoch + 1))
	if test_acc > max_test_acc:
		max_test_acc = test_acc
		temp_epoch = epoch + 1
		torch.save(net.state_dict(), 'best_new_3_model.pt')
...
net = arch()
net.load_state_dict(torch.load('best_new_3_model.pt', map_location='cpu'))
print('Model loaded !')
net = net.double().cuda()
...
np.save('all_3_new_test_acc.npy', np.array(testAcc))
np.save('all_3_new_val_acc.npy', np.array(valAcc))
np.save('all_3_new_train_acc.npy', np.array(trainAcc))

np.save('all_3_new_test_loss.npy', np.array(testLoss))
np.save('all_3_new_val_loss.npy', np.array(valLoss))
np.save('all_3_new_train_loss.npy', np.array(trainLoss))
# net.train(False)
