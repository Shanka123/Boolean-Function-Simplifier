...
class arch(nn.Module):
	def __init__(self):
		...
		self.block1 = nn.Sequential(nn.Conv2d(in_channels=1, out_channels=6, kernel_size=3, stride=1, padding=1), 
			...
			nn.ConvTranspose2d(in_channels = 6, out_channels = 3, kernel_size = 2, stride = 2), nn.ReLU())
...
for epoch in range(iterations):
	...
	for idx, data in enumerate(testLoader):
		...
		predicted = net(inputs)
		# print((time.time() - test_time_3)/BatchSize)
		loss = criterion(predicted, labels)
		runningLoss += loss.data[0]
		for i in range(predicted.size()[0]):
			running_correct+=compare(predicted[i], labels[i])
	...
	torch.save(net.state_dict(), 'running_model_3.pt')
	with open('present_epoch_3.txt','w') as f:
		f.write(str(epoch + 1))
	if test_acc > max_test_acc:
		max_test_acc = test_acc
		temp_epoch = epoch + 1
		torch.save(net.state_dict(), 'best_model_3.pt')
...
net = arch()
net.load_state_dict(torch.load('best_model_3.pt', map_location='cpu'))
print('Model loaded !')
net = net.double().cuda()
...
np.save('all_test_acc_3.npy', np.array(testAcc))
np.save('all_val_acc_3.npy', np.array(valAcc))
np.save('all_train_acc_3.npy', np.array(trainAcc))

np.save('all_test_loss_3.npy', np.array(testLoss))
np.save('all_val_loss_3.npy', np.array(valLoss))
np.save('all_train_loss_3.npy', np.array(trainLoss))
# net.train(False)
