...
num = np.arange(40000000).tolist()
req_num = np.asarray(random.sample(num, 500000))

input_arr = np.load('padded_kmap_all_input_data_new_x_terms.npy')[req_num]
print('input done')
target_arr = np.load('kmap_all_output_data_new_x_terms.npz')['a'][req_num]
print('output done')
input_arr = input_arr.astype(float)
input_arr[input_arr == float(2)] = 0.5
class K_map(Dataset):
...
class arch(nn.Module):
	def __init__(self):
		...
		self.pad_2x4 = nn.ZeroPad2d((1, 1, 0, 0))
		self.pad_4x2 = nn.ZeroPad2d((0, 0, 1, 1))
		self.pad_4x4 = nn.ZeroPad2d((1, 1, 1, 1))
		self.pad_1x4 = nn.ZeroPad2d((1, 1, 0, 0))
		self.pad_4x1 = nn.ZeroPad2d((0, 0, 1, 1))
	...
	def forward(self, x):
		...
		# exit()
		# x1 = self.pad_1x2(x)
		x1 = self.conv_1x2(x[:,:,0:4,:])
		x1 = F.relu(x1)
		# x2 = self.pad_2x1(x)
		x2 = self.conv_2x1(x[:,:,:,0:4])
		x2 = F.relu(x2)
		# x3 = self.pad_2x2(x)
		x3 = self.conv_2x2(x)
		x3 = F.relu(x3)
		...
		x6 = self.conv_4x4(x6)
		x6 = F.relu(x6)
		x7 = F.relu(self.conv_1x1(x[:,:,0:4,0:4]))
		x8 = F.relu(self.conv_1x4(self.pad_1x4(x[:,:,0:4,:])))
		x9 = F.relu(self.conv_4x1(self.pad_4x1(x[:,:,:,0:4])))
...
net = arch()
net.load_state_dict(torch.load('best_new_padded_x_model.pt', map_location='cpu'))
print('Model loaded !')
# net = net.double().cuda()
if use_gpu:
	net = net.double().cuda()
...
for epoch in range(iterations):
	...
	torch.save(net.state_dict(), 'running_new_padded_x_model_from_200.pt')
	with open('new_padded_x_present_epoch_from_200.txt','w') as f:
		f.write(str(epoch + 1))
	if test_acc > max_test_acc:
		max_test_acc = test_acc
		temp_epoch = epoch + 1
		torch.save(net.state_dict(), 'best_new_padded_x_model_from_200.pt')
...
net = arch()
net.load_state_dict(torch.load('best_new_padded_x_model_from_200.pt', map_location='cpu'))
print('Model loaded !')
net = net.double().cuda()
...
np.save('all_padded_x_new_test_acc_from_200.npy', np.array(testAcc))
np.save('all_padded_x_new_val_acc_from_200.npy', np.array(valAcc))
np.save('all_padded_x_new_train_acc_from_200.npy', np.array(trainAcc))

np.save('all_padded_x_new_test_loss_from_200.npy', np.array(testLoss))
np.save('all_padded_x_new_val_loss.npy_from_200', np.array(valLoss))
np.save('all_padded_x_new_train_loss.npy_from_200', np.array(trainLoss))
# net.train(False)
