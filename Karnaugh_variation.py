...
input_arr = np.load('kmap_all_input_data.npy')
...
class arch(nn.Module):
	def __init__(self):
		...
		self.conv_2x2 = nn.Conv2d(in_channels=1, out_channels=16, kernel_size=(2,2), stride=1)
		self.conv_2x4 = nn.Conv2d(in_channels=1, out_channels=8, kernel_size=(2,4), stride=1)
		self.conv_4x2 = nn.Conv2d(in_channels=1, out_channels=8, kernel_size=(4,2), stride=1)
		self.conv_4x4 = nn.Conv2d(in_channels=1, out_channels=4, kernel_size=(4,4), stride=1)

		self.fc = nn.Sequential(nn.Linear(in_features = 100*4*4, out_features = 129, bias = True))
	def forward(self, x):
		# print(x.size())
		# exit()
		x1 = self.pad_1x2(x)
...
for epoch in range(iterations):
	...
	torch.save(net.state_dict(), 'running_new_model.pt')
	with open('new_present_epoch.txt','w') as f:
		f.write(str(epoch + 1))
	if test_acc > max_test_acc:
		max_test_acc = test_acc
		temp_epoch = epoch + 1
		torch.save(net.state_dict(), 'best_new_model.pt')
...
net = arch()
net.load_state_dict(torch.load('best_new_model.pt', map_location='cpu'))
print('Model loaded !')
net = net.double().cuda()
...
np.save('all_new_test_acc.npy', np.array(testAcc))
np.save('all_new_val_acc.npy', np.array(valAcc))
np.save('all_new_train_acc.npy', np.array(trainAcc))

np.save('all_new_test_loss.npy', np.array(testLoss))
np.save('all_new_val_loss.npy', np.array(valLoss))
np.save('all_new_train_loss.npy', np.array(trainLoss))
# net.train(False)
