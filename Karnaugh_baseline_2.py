...
class arch(nn.Module):
	def __init__(self):
		...
		self.block1 = nn.Sequential(nn.Conv2d(in_channels=1, out_channels=3, kernel_size=3, stride=1, padding=1), 
			nn.ReLU(),
			# nn.Upsample(scale_factor=2, mode='bilinear'))
			nn.ConvTranspose2d(in_channels = 3, out_channels = 3, kernel_size = 2, stride = 2), nn.ReLU())
		self.fc = nn.Sequential(nn.Linear(in_features = 64*3, out_features = 129, bias = True))
	def forward(self, x):
		x = self.block1(x)
		x = x.view(-1,64*3)
		x = self.fc(x)
...
for epoch in range(iterations):
	...
	for idx, data in enumerate(testLoader):
		...
		predicted = net(inputs)
		# print((time.time() - test_time_2)/BatchSize)
		loss = criterion(predicted, labels)
		runningLoss += loss.data[0]
		for i in range(predicted.size()[0]):
			running_correct+=compare(predicted[i], labels[i])
	...
	torch.save(net.state_dict(), 'running_model_2.pt')
	with open('present_epoch_2.txt','w') as f:
		f.write(str(epoch + 1))
	if test_acc > max_test_acc:
		max_test_acc = test_acc
		temp_epoch = epoch + 1
		torch.save(net.state_dict(), 'best_model_2.pt')
...
net = arch()
net.load_state_dict(torch.load('best_model_2.pt', map_location='cpu'))
print('Model loaded !')
net = net.double().cuda()
...
np.save('all_test_acc_2.npy', np.array(testAcc))
np.save('all_val_acc_2.npy', np.array(valAcc))
np.save('all_train_acc_2.npy', np.array(trainAcc))

np.save('all_test_loss_2.npy', np.array(testLoss))
np.save('all_val_loss_2.npy', np.array(valLoss))
np.save('all_train_loss_2.npy', np.array(trainLoss))
# net.train(False)
