...
class arch(nn.Module):
	def __init__(self):
		...
		self.block1 = nn.Sequential(nn.Conv2d(in_channels=1, out_channels=12, kernel_size=3, stride=1, padding=1), 
			nn.LeakyReLU(0.1), nn.Dropout(0.1),
			# nn.Upsample(scale_factor=2, mode='bilinear'))
			nn.ConvTranspose2d(in_channels = 12, out_channels = 72, kernel_size = 2, stride = 2), nn.LeakyReLU(0.1), nn.Dropout(0.1))
		self.fc = nn.Sequential(nn.Linear(in_features = 64*72, out_features = 80, bias = True))
	def forward(self, x):
		x = self.block1(x)
		x = x.view(-1,64*72)
		x = self.fc(x)
...
for epoch in range(iterations):
	...
	torch.save(net.state_dict(), 'running_model_5.pt')
	with open('present_epoch_5.txt','w') as f:
		f.write(str(epoch + 1))
	if test_acc > max_test_acc:
		max_test_acc = test_acc
		temp_epoch = epoch + 1
		torch.save(net.state_dict(), 'best_model_5.pt')
...
net = arch()
net.load_state_dict(torch.load('best_model_5.pt', map_location='cpu'))
print('Model loaded !')
net = net.double().cuda()
...
np.save('all_test_5_acc.npy', np.array(testAcc))
np.save('all_val_5_acc.npy', np.array(valAcc))
np.save('all_train_5_acc.npy', np.array(trainAcc))

np.save('all_test_5_loss.npy', np.array(testLoss))
np.save('all_val_5_loss.npy', np.array(valLoss))
np.save('all_train_5_loss.npy', np.array(trainLoss))
# net.train(False)
