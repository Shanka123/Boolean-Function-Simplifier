...
class arch(nn.Module):
	def __init__(self):
		...
		self.conv_4x4 = nn.Conv2d(in_channels=1, out_channels=32, kernel_size=(4,4), stride=1)

		self.fc = nn.Sequential(nn.Linear(in_features = 32*6*4*4, out_features = 129, bias = True))
	def forward(self, x):
		# print(x.size())
		# exit()
		x1 = self.pad_1x2(x)
