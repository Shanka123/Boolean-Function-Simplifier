...
n_var = 5
num_list = np.arange(5105505,int((2**(2**n_var))/32)).tolist()
with open('kmap_5x5_data.txt', 'a') as f:
	for num in num_list:
			# driver.get('http://www.32x8.com/var4kmapn.html')
			# button_tuple = ()
		button_list = []
		a = format(num, '0' + str(2**n_var) + 'b')
		for i,item in enumerate(a):
			if item == '1':
				# button_tuple = button_tuple + (str(i),) #button_list is zero indexed.
				button_list.append(str(i))
		temp_inp = ','.join(button_list)
		eqn=main(n_var,temp_inp)
		f.write(temp_inp + '\t' + eqn + '\n')
		print(num)
